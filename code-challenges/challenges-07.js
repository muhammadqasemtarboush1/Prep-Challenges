...
const objLat = (obj) => {
    ...
    for (const key in obj) {
        if(key === "firstName"){
            str = "my name is" + " " +obj[key].charAt(0).toUpperCase() + obj[key].slice(1);
        }
        if(key === "lastName"){
            str += " " + obj[key].charAt(0).toUpperCase() + obj[key].slice(1);
        }
        if(key === "age"){
            str +=  ` I am ${obj[key]} YO, and I love `
        }
        if(key === "hobby"){
            str += obj[key] + "."
        }
        ...
        // for (const key in arr[i]) {
            ...
            if(arr[i]["yearsOfExperience"] >1){
                ...
                if(arr[i]["lastName"] !== null){
                    ...
                    // arrCV = arrCV.push({fullName: arr[i]["firstName"] + " " + arr[i]["lastName"],arr[i]["firstName"] + " " + arr[i]["lastName"]})
                }
                obj["tech"] = arr[i]["tech"]
            arrCV.push(obj)
            }      
