...
const roundDecimals = (arr) => {
  let result = [];
  arr.forEach((number) => {
    let digit = number.toString().split(".")[1];
    // console.log(number.toString().split(".")[1], "round");
    if (digit >= 5) {
      digit = parseInt(number.toString().split(".")[0]) + 1;
      result.push(digit);
    } else {
      result.push(parseInt(number.toString().split(".")[0]));
    }
  });
  return result;
};
