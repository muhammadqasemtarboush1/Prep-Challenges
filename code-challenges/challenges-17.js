...
const recursionPattern = (int1, int2) => {
  let arrOfPatteren = [];
  const instedFunction = (int1, int2) => {
    arrOfPatteren.push(int1);
    if (int1 < 0) {
      arrOfPatteren = arrOfPatteren.concat(
        arrOfPatteren.slice(0, arrOfPatteren.length - 1).reverse()
      );
      return arrOfPatteren;
    } else {
      int1 = instedFunction(int1 - int2, int2);
      return arrOfPatteren;
    }
  };
  instedFunction(int1, int2);
  return arrOfPatteren;
};
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
  // .com slove but not .org || .net
  let first = str.search("www");
  let last = "";
  if (str.search('">')) {
    last = str.search('">') - 16;
  }
...
const isPalindrome = (str) => {
  let regEx = /[\W_]/g;
  let loCaStr = str.toLowerCase();
  loCaStr = loCaStr.replace(regEx, "");
  let revStr = loCaStr.split("").reverse().join("");
  if (revStr === loCaStr) return true;
