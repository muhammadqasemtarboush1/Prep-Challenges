...
const customerAndAge = (obj) => {
  ...
  for (const [name, age] of Object.entries(obj)) {
    let costomer = `Customer Name :${name} , Age :${age}`;
    costomersArr.push(costomer);
  }
  return costomersArr;
};
// -------------------------------------------------------------------------------------------------------
...
const getInfo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    for (const [key, value] of Object.entries(arr[i])) {
      ...
      if (key === "Students" && value.length > 0) {
        // console.log(value.toString().split(","));
        for (let i = 0; i < value.length; i++) {
          let studentName = value[i];
          studentsName.push(studentName);
        }
        // let stringsNames = value.toString().split(",");
